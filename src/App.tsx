      ...
      <section id="about" style={{ background: '#101026', padding: '100px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}><div style={{ display: 'flex', alignItems: 'center', gap: 60, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 420px', minWidth: 300 }}>
            ...
            <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>{[{ num: '10+', label: 'שנות ניסיון', color: '#2B4CFF' }, { num: '500+', label: 'אירועים', color: '#FF007F' }, { num: '100%', label: 'אנרגיה', color: '#DFB04F' }].map(s => <div key={s.label} style={{ textAlign: 'center' }}><div className="font-display" style={{ fontSize: 44, fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.num}</div><div style={{ color: '#A0A2C3', fontSize: 13, marginTop: 6, fontWeight: 500 }}>{s.label}</div></div>)}</div>
          </div>
